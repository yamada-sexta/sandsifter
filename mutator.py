...
def process(i):
    ...
    sys.stdout.write(
        "%s ... %10s %-45s "
        % ("".join("{:02x}".format(ord(c)) for c in i.pad[:8]), i.mnemonic, i.op_str)
    )
    sys.stdout.flush()
    i.r = run(i.pad)
    sys.stdout.write(
        "%3d %3d %3d %3d" % (i.r.valid, i.r.length, i.r.signum, i.r.sicode)
    )
    sys.stdout.flush()
...
injector = subprocess.Popen(
    "./injector -d -R", shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE
)
...
while True:
    ...
    if s.r.valid:
        for i in range(1, MUTATIONS):
            ...
            if t.r.valid:
                if (
                    s.r.length != t.r.length
                    or s.r.signum != t.r.signum
                    or s.r.sicode != t.r.sicode
                ):
                    q.append(t)
                    found_new = True
                    sys.stdout.write(" !")
