...
# prefixes)
def disas_objdump(b):
    with open("/dev/shm/shifter", "wb") as f:
        f.write(b)
    if arch == "64":
        ...
        dis, errors = subprocess.Popen("objdump -D --insn-width=256 -b binary \
                ...
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True).communicate()
    dis = dis.decode('utf-8')[6:] # address
...
def result_string(insn, result):
    s = "%30s %2d %2d %2d %2d (%s)\n" % (
            hexlify(insn).decode('ascii'), result.valid,
            result.length, result.signum,
            result.sicode, hexlify(cstr2py(result.raw_insn)).decode('ascii'))
    return s
        ...
        try:
            ...
            self.T.il.append(
                    (
                        ...
                        op_str,
                        self.T.r.length,
                        "%s" % hexlify(synth_insn).decode('ascii')
                    )
                )
            ...
            if self.maxy >= top + top_bracket_height + 5 + self.T.UL + 2:
                ...
                try:
                    for (i, r) in enumerate(self.T.al):
                        ...
                        insn_hex = hexlify(cstr2py(r.raw_insn)).decode('ascii')
...
def cleanup(gui, poll, injector, ts, tests, command_line, args):
    ...
    if args.save:
        with open(LAST, "w") as f:
            f.write(hexlify(cstr2py(tests.r.raw_insn)).decode('ascii'))
...
def main():
    ...
    injector_bitness = injector_bitness.decode('utf-8')
