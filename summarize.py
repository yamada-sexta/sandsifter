...
import os
import locale
from typing import Literal, Optional, TypedDict, Tuple, Dict
from gui.gui import *
...
CondensedResult = namedtuple(
    "CondensedResult", "raw valids lengths signums sicodes prefixes"
)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("You need to specify a log file!")
        exit(1)
    ...
    def bin(instructions, index, base="", bin_progress=0, progress_out_of=None):
        ...
        for b in c.d:
            (c.d[b], bin_progress) = bin(
                c.d[b], index + 1, base + chr(b), bin_progress, progress_out_of
            )
    ...
    def build_instruction_summary(c, index=0, summary=None, lookup=None):
        ...
        if c.count > 1:
            ...
            suffix = (
                ".." * (min(c.lengths) - len(c.base))
                + " "
                + ".." * (max(c.lengths) - min(c.lengths))
            )
            summary.append(
                "  " * index + "> " + hexlify(c.base.encode()).decode() + suffix
            )
            if not c.collapsed:
                for b in sorted(c.d):
    ...
    def draw_infobox(gui: Gui, o):
        ...
        if type(o) == Catalog:
            ...
            gui.window.addstr(
                line, infobox_x + 2, "instruction group:", curses.color_pair(gui.RED)
            )
            line = line + 1

            g = hexlify(o.base.encode()).decode()
            if not g:
                g = "(all)"
            ...
            gui.window.addstr(
                line, infobox_x + 2, "instructions found in this group:", gui.gray(0.5)
            )
            line = line + 1
            gui.window.addstr(line, infobox_x + 2, "%d" % o.count, gui.gray(0.8))
            line = line + 1
            ...
            gui.window.addstr(
                line,
                infobox_x + 2,
                "example instruction from this group:",
                gui.gray(0.5),
            )
            line = line + 1
            gui.window.addstr(
            ...
            gui.window.addstr(
                line, infobox_x + 2, "group attribute summary:", gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "valid:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_valids(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "length:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_lengths(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "signum:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_signums(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "signal:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_signames(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "sicode:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_sicodes(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "prefixes:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_prefixes(o), gui.gray(0.8)
            )
            line = line + 1
            ...
            gui.window.addstr(
                line, infobox_x + 2, "instruction:", curses.color_pair(gui.RED)
            )
            line = line + 1
            gui.window.addstr(
                line, infobox_x + 2, "%-30s" % hexlify(o.raw).decode(), gui.gray(1)
            )
            line = line + 1
            ...
            gui.window.addstr(line, infobox_x + 2, "valids:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_valids(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "lengths:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_lengths(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "signums:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_signums(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "signals:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_signames(o), gui.gray(0.8)
            )
            line = line + 1

            gui.window.addstr(line, infobox_x + 2, "sicodes:", gui.gray(0.5))
            gui.window.addstr(
                line, infobox_x + 18, "%-30s" % summarize_sicodes(o), gui.gray(0.8)
            )
            line = line + 1
            ...
            gui.window.addstr(
                line, infobox_x + 2, "analysis:", curses.color_pair(gui.RED)
            )
            line = line + 1
            ...
            for disassembler in sorted(disassemblers):
                ...
                if disassembler == CAPSTONE:
                    ...
                    (asm, raw) = disassemble(
                        disassembler, processor.architecture, dis_data
                    )
                if not asm:
                    asm = "(unknown)"
                if not raw:
                ...
                gui.window.addstr(
                    line, infobox_x + 2, "%s:" % disassembler, gui.gray(0.5)
                )
                line = line + 1

                gui.window.addstr(line, infobox_x + 4, "%-30s" % asm, gui.gray(0.8))
                line = line + 1
                gui.window.addstr(line, infobox_x + 4, "%-30s" % raw, gui.gray(0.5))
                line = line + 1
    ...
    while True:
        ...
        gui.window.addstr(2, 1, detail_string, gui.gray(0.6))
        ...
        if key == 0x0A:
            ...
            textbox.scroll_up()
            while (
                not textbox.at_top() and type(lookup[textbox.selected_index]) != Catalog
            ):
                textbox.scroll_up()
                smooth_scroll()
        elif key == ord("}"):
            textbox.scroll_down()
            while (
                not textbox.at_bottom()
                and type(lookup[textbox.selected_index]) != Catalog
            ):
                textbox.scroll_down()
                smooth_scroll()
        elif key == ord("q"):
